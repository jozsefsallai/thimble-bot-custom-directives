 ...
 */
const log = str => {
  if (config.custom.MovieTracker.debug) {
    console.log(str);
  }
};
...
const MovieTracker = async () => {
  const port = new PortHuCrawler({
    channels: config.custom.MovieTracker.portdothu.channels,
    genres: config.custom.MovieTracker.portdothu.genres
  });

  const cinemagia = new CinemagiaCrawler({
    channels: config.custom.MovieTracker.cinemagia.channels,
    genre: config.custom.MovieTracker.cinemagia.genre
  });
